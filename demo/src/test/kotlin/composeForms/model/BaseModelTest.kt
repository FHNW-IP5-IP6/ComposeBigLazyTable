...
internal class BaseModelTest {
    ...
    fun setUp(){
        //reset
        Attribute.resetId()
        model = object: BaseModel<Label>(Label.TITLE){}

        //given
        age = IntegerAttribute(model = model, value = AGE, label = Label.AGE)

        noOfChildren = IntegerAttribute(model = model, value = NO_OF_CHILDREN, label = Label.NO_OF_CHILDREN)

        group = Group(model, Label.G1, Field(age), Field(noOfChildren))
        group1 = Group(model, Label.G2, Field(age))
        group2 = Group(model, Label.G3)
    }
    ...
    fun testResetWizardMode(){
        //given
        model = object: BaseModel<Label>(Label.TITLE, wizardMode = true){}
        val attr = StringAttribute(model, Label.TEST)
        age = IntegerAttribute(model, Label.TEST, value = AGE)
        val g1 = Group(model, Label.G1, Field(age))
        Group(model, Label.G2, Field(attr))
    ...
    fun testGetAttributeType(){
        //when
        var attr: Attribute<*, *, *> = StringAttribute(model, Label.NO_OF_CHILDREN)
        //then
        assertEquals(AttributeType.STRING, model.getAttributeType(attr))

        //when
        attr = DoubleAttribute(model, Label.NO_OF_CHILDREN)
        //then
        assertEquals(AttributeType.DOUBLE, model.getAttributeType(attr))

        //when
        attr = FloatAttribute(model, Label.NO_OF_CHILDREN)
        //then
        assertEquals(AttributeType.FLOAT, model.getAttributeType(attr))

        //when
        attr = ShortAttribute(model, Label.NO_OF_CHILDREN)
        //then
        assertEquals(AttributeType.SHORT, model.getAttributeType(attr))

        //when
        attr = IntegerAttribute(model, Label.NO_OF_CHILDREN)
        //then
        ...
        //when
        attr = SelectionAttribute(model, Label.NO_OF_CHILDREN, emptyList())
        //then
    ...
    fun testOnChangeListeners(){
        ...
        Group(model, Label.G1, Field(taxNumber))
    ...
    fun testSetCurrentFocusIndex(){
        ...
        //given
        val notInModelAttr = IntegerAttribute(object: BaseModel<Label>(Label.TITLE){}, Label.TEST)
    ...
    fun testFocusWithHeaderGroup(){
        //given
        val attr1 = StringAttribute(model, Label.TEST)
        val attr2 = StringAttribute(model, Label.TEST)
        val attr3 = StringAttribute(model, Label.TEST)

        val hg = HeaderGroup(model, Label.GHeader, Field(attr1), Field(attr2), Field(attr3))
    ...
    fun testWizardGroupChanges(){
        ...
        model = object: BaseModel<Label>(Label.TITLE, wizardMode = true){}
        val attr1 = StringAttribute(model, Label.TEST)
        val attr2 = IntegerAttribute(model, Label.TEST, value = AGE)
        val attr3 = DoubleAttribute(model, Label.TEST, value = 0.0)
        val g1 = Group(model, Label.G1, Field(attr1))
        val g2 = Group(model, Label.G2, Field(attr2))
        val g3 = Group(model, Label.G3, Field(attr3))
    ...
    fun testNextPreviousInWizardMode(){
        ...
        val attr2 = IntegerAttribute(model, Label.TEST, value = AGE)
        val attr3 = DoubleAttribute(model, Label.TEST, value = 0.0)
        val attr4 = DoubleAttribute(model, Label.TEST, value = 0.0)
        val g1 = Group(model, Label.G1, Field(attr1), Field(attr2))
        val g2 = Group(model, Label.G2, Field(attr3), Field(attr4))
        model.setCurrentFocusedAttribute(attr1, g1)
    ...
    fun testOnReceivedCommand(){
        //given
        val attr = StringAttribute(model, Label.NO_OF_CHILDREN)
    ...
    fun testWizardMode(){
        //given
        val wizardModel = object: BaseModel<Label>(Label.TITLE, wizardMode = true){}
        age             = IntegerAttribute(model = wizardModel, value = AGE, label = Label.AGE)
        noOfChildren    = IntegerAttribute(model = wizardModel, value = NO_OF_CHILDREN, label = Label.NO_OF_CHILDREN)
        group           = Group(wizardModel, Label.G1, Field(age), Field(noOfChildren))
        group1          = Group(wizardModel, Label.G2, Field(age))
    ...
    fun testWizardModeWithInitiallyInvalidAttribute(){
        //given
        val wizardModel = object: BaseModel<Label>(Label.TITLE, wizardMode = true){}
        age             = IntegerAttribute(model = wizardModel, label = Label.AGE, required = true) //invalid
        noOfChildren    = IntegerAttribute(model = wizardModel, value = NO_OF_CHILDREN, label = Label.NO_OF_CHILDREN) //valid
        group           = Group(wizardModel, Label.G1, Field(age), Field(noOfChildren))
        group1          = Group(wizardModel, Label.G2, Field(age))
