 ...
 * It also provides a mqtt server to allow a smartphone connection.
 * Inherit from this class and add attributes to it.
 *
 * @param title: Title as [L] for the form
 * @param smartphoneOption: starts the mqtt server if the boolean is true
...
abstract class BaseModel<L>(private val title: L,
                            ...
                            ) : IModel<L> where L: ILabel, L: Enum<*> {
    ...
    /**
     * This method adds a new group to the model and reinitializes and validates the currentWizardGroup if it is set to null.
     *
    ...
    /**
     * Checking if [attr] is from this model and in the [group]
     *
     * @param attr: Attribute
     * @param group : Group
     */
    ...
    /**
     * This method starts an embeded MQTT server in a separate scope.
     */
    ...
    /**
     * Call [MqttConnector.connect] to connect to the server and subscribe the subtopics text and command.
     */
    ...
    /**
     * Subscribe to all channels that the model is listening to and set the corresponding function calls
     */
